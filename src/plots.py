...
def plot_patient_variables_grid(
    ...
    *,
    ...
    fig, axes = plt.subplots(
        nrows=nrows, ncols=1, figsize=((16 * 2.3 * nrows) / 9, 2.3 * nrows), sharex=False
    )
    if nrows == 1:
        axes = [axes]
    ...
    y0, dy = 0.07, 0.01  # starting y and spacing between lines
    # y0, dy = 0.15, 0.01   # starting y and spacing between lines
    for i, (lab, col) in enumerate(anom_specs):
        h = Line2D(
            [0],
            [0],
...
def plot_all_patients_in_parallel():
    df = pd.read_csv(Path(INTERIM_DATA_DIR / "ALL_PRESCRIPTION_DATA_FILTERED.csv"))
    df["Prescription Date"] = pd.to_datetime(
        df["Prescription Date"], format="%Y%m%d", errors="coerce"
    )
    df["Result Numerical Value"] = pd.to_numeric(df["Result Numerical Value"], errors="coerce")
