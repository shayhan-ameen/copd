...
class TimeSeriesTransformerRegressor(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # --- output clipping (eval-time only) ---
        self.clip_at_eval = False
        self.register_buffer("y_min_bound", torch.tensor(float("-inf")))
        self.register_buffer("y_max_bound", torch.tensor(float("inf")))

    def set_output_bounds(self, lo: float, hi: float):
        assert hi > lo
        self.y_min_bound.fill_(lo)
        self.y_max_bound.fill_(hi)

    def enable_output_clipping(self, flag: bool = True):
        self.clip_at_eval = flag
    ...
    def forward(self, X: Tensor, M: Tensor, DT: Tensor, lengths: Tensor) -> Tensor:
        ...
        # NEW: clip only during evaluation to avoid hurting training
        if (not self.training) and self.clip_at_eval:
            y_hat = y_hat.clamp(min=self.y_min_bound, max=self.y_max_bound)

        # NEW: clip only during evaluation to avoid hurting training
        # if (not self.training) and self.clip_at_eval:
        #     y_hat = torch.clamp(
        #         y_hat,
        #         min=self.y_min_bound.item(),
        #         max=self.y_max_bound.item(),
        #     )
...
def run_k_fold_cv_earlystop(
    ...
    *,
    ...
    pids_all = np.array(
        [
            (ds[i]["pid"] if isinstance(ds[i], dict) and "pid" in ds[i] else None)
    ...
    # Infer base D (to set model input_size) from a single sample
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        #! Adjust capacity: d_model (e.g., 128→96), num_layers (2–4), nhead (divides d_model), dim_feedforward.

        model = TimeSeriesTransformerRegressor(
            input_size=Din,  # Din = D or D*2 if you set concat_XM=True in your loop
            d_model=128,  # must be divisible by nhead #! 96
            nhead=8,
        ...
        y_train_vals = np.array([float(ds[i]["y"].item()) for i in train_idx])  # outer-train only
        model.set_output_bounds(y_train_vals.min(), y_train_vals.max())
        model.enable_output_clipping(True)
        ...
        # --- NEW: save per-fold test predictions with pids ---
        y_te_true, y_te_pred = predict(model, test_loader, device)
        df_pred = pd.DataFrame(
            {
                "pid": pids_all[test_idx],
                "y_true": y_te_true,
                "y_pred": y_te_pred,
            }
        )
        df_pred.to_csv(fold_dir / "test_predictions.csv", index=False)  # per-fold file
        df_pred["fold"] = fold
    ...
    # --- NEW: concat all folds and sort by absolute error once ---
