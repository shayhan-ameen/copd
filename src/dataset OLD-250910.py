...
def summarize_pft_df(df: pd.DataFrame, show_values: bool = True) -> Dict[str, Any]:
    ...
    combo_counts_mt = (
        mt_base.value_counts(["Measurement", "Test"])
    ...
    # ---- (Measurement, Test) value counts (cleaned) ----
    mt_base = df.loc[
        df["Measurement"].notna() & df["Variable"].notna(), ["Measurement", "Variable"]
    ].copy()
    mt_base["Measurement"] = mt_base["Measurement"].astype(str).str.strip()
    mt_base["Variable"] = mt_base["Variable"].astype(str).str.strip()

    combo_counts_mv = (
        mt_base.value_counts(["Measurement", "Variable"])
        .reset_index(name="count")
    ...
    print(
        f"5) Total unique (measurement, test) combinations: {result['total_unique_(measurement,test)_combinations']}"
    )
    if show_values:
...
def detect_anomalies(
    ...
    Flags:
      ...
      - Anomaly_Range: outside clinical ranges
          ...
          FEV1/FVC ratio (Meas/Post_Meas): 0.1–120%
    ...
    jump_thresh_per_month = {
        "Meas": {"FEV1": 0.30, "FVC": 0.40, "DLCO": 3.0, "FEV1/FVC": 8.0, "DLCO/VA": 0.60},
    ...
    df2 = dfx.copy()
    pid = df2["Patient Number"].iloc[0] if "Patient Number" in df2 else "Unknown"

    # Ensure numeric + datetime for calculations
    v = pd.to_numeric(df2[value_col], errors="coerce")
    dt = pd.to_datetime(df2[date_col], format="%Y%m%d", errors="coerce")
    df2["_val_"] = v
    df2["_date_"] = dt
    ...
    rng_flag |= _violate(mask_abs & (m == "FEV1/FVC"), 0.1, 120)
    ...
    for tmv, g in df2.groupby(
        [test_col, measure_col, variable_col], dropna=False
    ...
    df2["Outlier_MAD_JUMP"] = df2["Outlier_MAD"] & df2["Outlier_Jump"]

    # Compose row-wise tags
    def _row_tags(row):
        tags = []
        if row["Anomaly_Missing"]:
            tags.append("Missing value")
        if row["Anomaly_Range"]:
            tags.append("Out of valid range")
        if row["Outlier_MAD"]:
            tags.append("MAD")
        if row["Outlier_Jump"]:
            tags.append("Jump")
    ...
    if df2["Anomaly_Missing"].any():
        present.append("Missing value")
    if df2["Anomaly_Range"].any():
        present.append("Out of valid range")
    if df2["Outlier_MAD"].any():
        present.append("MAD")
    if df2["Outlier_Jump"].any():
        present.append("Jump")
            ...
            audit_rows.append(
                {
                    ...
                    "unknown_columns_not_in_mapping": " | ".join(map(str, unknown_cols))
                    if unknown_cols
                    else "",
                    "mapping_columns_missing_in_file": " | ".join(map(str, missing_cols))
                    if missing_cols
                    else "",
...
def filter_relevant_measurements(
    ...
    def has_min_3_years(g):
        dates = pd.to_datetime(g["Prescription Date"], format="%Y%m%d", errors="coerce")
        if dates.isnull().all():
            return False
